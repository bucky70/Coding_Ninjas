...
public class Solution {
    ...
    public static BinaryTreeNode<Integer> replace(BinaryTreeNode<Integer> root,int level){
        if(root==null){
            return null;
        }
        root.data=level;
        root.left=replace(root.left,level+1);
        root.right=replace(root.right,level+1);
        return root;
    }
}